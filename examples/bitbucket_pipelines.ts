import {ResolvedJsonSchema} from "../src/all_types/json_schema/json_schema";
    ...
    pipelines: {
      ...
      properties: {
        ...
        bookmarks: {
          type: "object",
          title:
            "Bookmark-specific build pipelines (to be used in Mercurial repositories)",
          description:
    ...
    caches: {
      ...
      patternProperties: {
        "^(?!-)[-a-z0-9]{0,49}[a-z0-9]$": {
          ...
          description:
            "Path to the directory to be cached, can be absolute or relative to the clone directory",
          minLength: 1,
        },
      },
      not: {
...
type BitbucketPipeline = ResolvedJsonSchema<typeof schema>;

const example: BitbucketPipeline = {
  image: "node:10.15.0",
  pipelines: {
    custom: {
      sonar: [
        {
          step: {
            script: ['echo "Manual triggers for Sonar are awesome!"'],
          },
        },
      ],
      "deployment-to-prod": [
        {
          step: {
            script: ['echo "Manual triggers for deployments are awesome!"'],
          },
        },
      ],
    },
    branches: {
      staging: [
        {
          step: {
            script: ['echo "Auto pipelines are cool too."'],
          },
        },
      ],
    },
